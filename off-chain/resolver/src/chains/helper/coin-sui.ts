...
export class SuiCoinHelper {
    ...
    public static readonly MIST_PER_SUI = BigInt(10 ** SuiCoinHelper.SUI_DECIMALS);
     ...
     */
    async getCoins(coinType: string = SuiCoinHelper.SUI_TYPE): Promise<CoinInfo[]> {
     ...
     */
    async getTotalBalance(coinType: string = SuiCoinHelper.SUI_TYPE): Promise<bigint> {
     ...
     */
    async getLargestCoin(coinType: string = SuiCoinHelper.SUI_TYPE): Promise<CoinInfo | null> {
    ...
    async splitCoin(
        ...
        tx.transferObjects([splitCoins].flat(), this.keypair.getPublicKey().toSuiAddress());
    ...
    async transferCoins(
        ...
        if (coinType === SuiCoinHelper.SUI_TYPE) {
            ...
            if (selectedCoins.length === 1 && selectedCoins[0].balance === amount) {
                ...
                tx.transferObjects([tx.object(selectedCoins[0].coinObjectId)], recipient);
            } else {
                // Need to merge and/or split
                const primaryCoin = selectedCoins[0];
                const coinsToMerge = selectedCoins.slice(1).map(coin => coin.coinObjectId);
                ...
                const [coinToTransfer] = tx.splitCoins(tx.object(primaryCoin.coinObjectId), [amount]);
    ...
    static mistToSui(mistAmount: bigint): string {
        ...
        const fractionStr = suiFraction.toString().padStart(SuiCoinHelper.SUI_DECIMALS, '0');
