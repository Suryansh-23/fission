...
import { SuiDstEscrowHelper } from "./dst-escrow";
...
export class SuiClient {
  ...
  constructor(config: SuiConfig) {
    ...
    this.immutablesHelper = new SuiImmutablesHelper(
      this.client,
      this.keypair,
      config.packageId
    );
    this.dstEscrowHelper = new SuiDstEscrowHelper(
      this.client,
      this.keypair,
      config.packageId
    );
    this.orderHelper = new SuiOrderHelper(
      this.client,
      this.keypair,
      config.packageId
    );
  ...
  async setResolverCapId() {
    const fullType = `${this.config.packageId}::resolver::ResolverCap`;
    const ids: string[] = [];
    ...
    this.resolverCapId = ids[0];
    console.log(`Resolver capability ID set: ${this.resolverCapId}`);
  }
  ...
  getResolverCapId(): string {
    if (!this.resolverCapId) {
      throw new Error(
        "Resolver capability ID not set. Call setResolverCapId() first."
      );
    }
  ...
  async resolveCoinTypeFromPkg(owner: string, pkgId: string): Promise<string> {
    ...
    for (const t of matches) {
      const meta = await this.client.getCoinMetadata({ coinType: t });
  ...
  async deploySrcEscrow(
    ...
    try {
      ...
      console.log(
        `Sui source escrow deployed via resolver - TxHash: ${result.digest}`
      );
  ...
  async createDstEscrow(
    ...
    try {
      ...
      const finalDstCancellation =
        dstCancellationTimestamp || now + 552n * seconds;
      const finalSrcCancellation =
        srcCancellationTimestamp || now + 888n * seconds;
      ...
      //     selectedCoins[0].balance === depositAmount
      //   ) {
      ...
      // -- new logic for deposit coin handling
      const coinIdWType = await this.resolveCoinTypeFromPkg(
        this.keypair.getPublicKey().toSuiAddress(),
        coinType
      );

      console.log(`[SuiClient] Coin type resolved: ${coinIdWType}`);

      const exactDeposit = tx.add(
        coinWithBalance({
          type: coinIdWType,
      ...
      console.log(
        `[SuiClient] Exact deposit prepared: ${JSON.stringify(exactDeposit)}`
      );

      const exactSafetyDeposit = tx.add(
        coinWithBalance({
      ...
      console.log(
        `[SuiClient] Exact safety deposit prepared: ${JSON.stringify(
          exactSafetyDeposit
        )}`
      );
      ...
      console.log(
        "[SuiClient] Calling resolver contract to create destination escrow"
      );
      console.log(`[SuiClient]`);

      console.log(`[SuiClient]   params:`);
      console.log(`[SuiClient]     coinIdWType: ${coinIdWType}`);
      console.log(`[SuiClient]     orderHash: ${Array.from(orderHash)}`);
      console.log(`[SuiClient]     hashlock: ${Array.from(hashlock)}`);
      console.log(`[SuiClient]     maker: ${maker}`);
      console.log(`[SuiClient]     taker: ${taker}`);
      console.log(`[SuiClient]     finalDstWithdrawal: ${finalDstWithdrawal}`);
      console.log(
        `[SuiClient]     finalDstPublicWithdrawal: ${finalDstPublicWithdrawal}`
      );
      console.log(
        `[SuiClient]     finalDstCancellation: ${finalDstCancellation}`
      );
      console.log(
        `[SuiClient]     finalSrcCancellation: ${finalSrcCancellation}`
      );

      tx.moveCall({
        target: `${this.config.packageId}::resolver::create_dst_escrow`,
        typeArguments: [coinIdWType],
        arguments: [
          tx.object("0x6"), // clock
          ...
          exactDeposit, // deposit: Coin<T>
          exactSafetyDeposit, // safety_deposit: Coin<SUI>
      ...
      if (dryRunResult.executionErrorSource) {
        throw new Error(`Dry run failed: ${dryRunResult.executionErrorSource}`);
      }
  ...
  async withdrawFromSrcEscrow(
    ...
    try {
      ...
      console.log(
        `[SuiClient] Source escrow withdrawal completed - TxHash: ${result.digest}`
      );
      return {
        txHash: result.digest,
  ...
  async withdrawFromDstEscrow(
    ...
    try {
      ...
      console.log(
        `[SuiClient] Destination escrow withdrawal completed - TxHash: ${result.digest}`
      );
      return {
        txHash: result.digest,
        blockHash: result.digest,
      ...
      console.error(
        "[SuiClient] Error withdrawing from destination escrow:",
        error
      );
      throw error;
    }
  ...
  async cancelOrder(
    ...
    try {
      ...
      console.log(
        `${side} escrow cancelled successfully - TxHash: ${result.digest}`
      );
  ...
  private extractCoinTypeFromObjectType(objectType: string): string {
    ...
    return match ? match[1] : SuiCoinHelper.SUI_PACKAGE_ID;
  }
